...
class VesselFactory{
	public:
		...
		Vessel* requestVessel(vType ctype, int segmentID);
		int getNumVesselPresent() const;
		std::string getnameVesselFactory() const;
	protected:
		Vessel* createVessel(vType ctype, int segmentID);// thiis method is not virtual it can be instatiated 
	private:
		int mNumVesselsPresent;
		std::string _nameVesselFactory;
};

Vessel* VesselFactory::requestVessel(vType ctype, int segmentID){
	...
	mNumVesselsPresent++;
	return createVessel(ctype, segmentID);
}
